...
// /subscribe/success
// PayPal redirects here after pastor approves the subscription.
// We show confirmation and the license activates automatically via webhook.
...
export default function SubscribeSuccessPage() {
  ...
  useEffect(() => {
    ...
    // Poll license status — PayPal webhook typically fires within 5-15 seconds
    let attempts = 0;
    const interval = setInterval(async () => {
      ...
      try {
        ...
        if (data.status === 'active') {
          ...
          // After 40 seconds, stop polling — webhook may still be coming
          clearInterval(interval);
        }
      } catch {
    ...
    <main className="min-h-screen bg-slate-950 flex items-center justify-center px-4">
      <div className="max-w-md w-full text-center">
        ...
        <p className="text-slate-400 mb-2">
          <span className="text-teal-400 font-semibold">{planLabel} plan</span>
        ...
        {status === 'active' && (
          <div className="bg-teal-500/10 border border-teal-500/30 rounded-xl p-5 mb-6 text-left">
            ...
            <p className="text-slate-400 text-sm">
              ...
              You can now access all features in your admin dashboard.
            </p>
          </div>
        )}
        ...
        <div className="flex flex-col gap-3">
          <Button
            asChild
            className="bg-teal-500 hover:bg-teal-400 text-white font-semibold w-full"
          >
