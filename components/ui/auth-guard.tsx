...
export function AuthGuard({ children }: { children: React.ReactNode }) {
  ...
  const now = new Date();
  const trialExpired = !loading && !!user && license?.status === 'trial' &&
    !!license.trial_ends_at && new Date(license.trial_ends_at) < now;
  const licenseExpired = !loading && !!user &&
    (license?.status === 'expired' || license?.status === 'suspended');

  useEffect(() => {
    if (trialExpired) router.replace('/trial-expired');
    else if (licenseExpired) router.replace('/license-expired');
  }, [trialExpired, licenseExpired]);

  if (loading || trialExpired || licenseExpired) {
    return (
